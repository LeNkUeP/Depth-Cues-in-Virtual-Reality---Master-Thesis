...
public class DepthCuePanel_old : MonoBehaviour
{
    ...
    public float convergenceMaxDistance = 2;
    ...
    public void CreateBlurClonesForAffectedObjects()
    {
        ...
        foreach (GameObject originalObject in objectsAffectedByConvergence)
        {
            var originalFilter = originalObject.GetComponent<MeshFilter>();
            ...
            // Abstand berechnen für Blur Stärke
            float distance = Vector3.Distance(Camera.main.transform.position, clone.transform.position);
            ...
            {
                float powerMin = 0f;
                float powerMax = 0.0045f;

                // Normalisieren und auf Shaderbereich mappen
                float normalized = Mathf.InverseLerp(convergenceMaxDistance, 0f, distance); // 2m -> 0, 0m -> 1
    ...
    public void CreateBlurClones()
    {
        ...
        {
            if (!originalRenderer.enabled)
                continue;

            var originalGO = originalRenderer.gameObject;
            var originalFilter = originalGO.GetComponent<MeshFilter>();

            if (originalFilter == null || originalFilter.sharedMesh == null)
                continue;

            // ----- Neuen visuellen Klon erzeugen -----
            GameObject clone = new GameObject(originalGO.name + "_BlurClone");

            // Parent setzen (wichtig für parallele Bewegung)
            clone.transform.SetParent(originalGO.transform, false);
            ...
            else
            {
                ...
                // Normalisieren und auf Shaderbereich mappen
                float normalized = Mathf.InverseLerp(2f, 0f, distance); // 2m -> 0, 0m -> 1
                float power = Mathf.Lerp(powerMin, powerMax, normalized);
